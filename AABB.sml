...
struct
  ...
  fun pading x =
    if Interval.size x > pad_delta then x else Interval.expand x pad_delta

  fun create (a: Interval.t) (b: Interval.t) (c: Interval.t) =
    {x = pading a, y = pading b, z = pading c}
  ...
  fun is_hit (aabb: {x: real * real, y: real * real, z: real * real})
    (ray: Ray.t) (initial_t_min: real, initial_t_max: real) : bool =
    let
      val {x = (ax_min, ax_max), y = (ay_min, ay_max), z = (az_min, az_max)} =
        aabb
       ...
       *)
      fun process_one_axis
        ( axis_min_val: real
        , axis_max_val: real
        , dir_one_axis: real
        , orig_one_axis: real
        , current_t_min_input: real
        , current_t_max_input: real
        ) : (real * real * bool) =
        if Real.== (dir_one_axis, 0.0) then (* レイが軸に平行な場合 *)
          ...
          let
            ...
            val (slab_entry_t, slab_exit_t) =
              if invDir < 0.0 then (t1, t0) else (t0, t1)
      ...
      (* X軸の処理 *)
      val (t_min_after_x, t_max_after_x, valid_x) = process_one_axis
        (ax_min, ax_max, ray_dir_x, ray_orig_x, initial_t_min, initial_t_max)
  ...
  fun longest_axis (aabb: t) =
    ...
    in
      if ax >= ay andalso ax >= az then 0
      else if ay >= ax andalso ay >= az then 1
      else 2
    end;
