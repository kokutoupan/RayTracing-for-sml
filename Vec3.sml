...
struct
  ...
  fun scale (v: t) (s: real) =
    {x = #x v * s, y = #y v * s, z = #z v * s}

  fun scaleV (v: t) (s: t) =
    {x = #x v * #x s, y = #y v * #y s, z = #z v * #z s}

  fun divide (v: t) (d: real) =
    {x = #x v / d, y = #y v / d, z = #z v / d}
  ...
  fun random_unit_vector () =
    let
      ...
      val l = length_sq p
    in
      if (1.0e~160 < l andalso l <= 1.0) then divide p (Math.sqrt l)
  ...
  fun refract (uv: t) (n: t) (eta_ratio: real) =
    let
      ...
      val r_out_parallel = scale n
        (Math.sqrt (Real.abs (1.0 - length_sq r_out_perp)) * ~1.0)
    in
...
struct
  ...
  fun write_color (out: TextIO.outstream) (color: t) =
    let
      ...
      val line =
        Int.toString ir ^ " " ^ Int.toString ig ^ " " ^ Int.toString ib ^ "\n"
    in
