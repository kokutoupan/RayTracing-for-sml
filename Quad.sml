...
struct
  ...
  local
    ...
    fun is_interior (a: real) (b: real) =
      ...
      in
        if
          Interval.contains unit_interval a
          andalso Interval.contains unit_interval b
        then true
        else false
      end
          ...
          in
            if not (Interval.contains t_ren t) then
              ...
              let
                ...
                val alpha = Vec3.dot w
                  (Vec3.cross planar_hitpt_vector (#v quad));
                val beta = Vec3.dot w (Vec3.cross (#u quad) planar_hitpt_vector)
              in
                if (not (is_interior alpha beta)) then
                  NoHit
                else
                  let
