structure Camera = struct
  ...
  type settings = {
    ...
    max_depth : int,
    vfov : real, (* Vertical view angle (field of view) *)
    lookfrom : Vec3.t, (* Point camera is looking from *)
    lookat : Vec3.t,   (* Point camera is looking at *)
  ...
  fun create (s : settings) : camera_params =
    let
      val image_height = Real.toInt IEEEReal.TO_NEAREST (Real.fromInt (#image_width s) / #aspect_ratio s)

      val camera_center = #lookfrom s
      val h = Math.tan( (Common.degrees_to_radians (#vfov s)) / 2.0 )

      val viewport_height = 2.0 * h * #focus_dist s
      val viewport_width = (Real.fromInt (#image_width s)) / (Real.fromInt image_height) * viewport_height
      ...
      val viewport_upper_left =
        Vec3.sub
          (Vec3.sub
            (Vec3.sub camera_center (Vec3.scale w (#focus_dist s)))
            (Vec3.divide viewport_u 2.0))
          (Vec3.divide viewport_v 2.0)

      val pixel00_loc = Vec3.add viewport_upper_left (Vec3.scale (Vec3.add pixel_delta_u pixel_delta_v) 0.5)

      val defocus_radius = (#focus_dist s) * (Math.tan (Common.degrees_to_radians ((#defocus_angle s)/2.0)))
    ...
    let
      ...
      fun defocus_disk_sample center () =
          ...
          in
            Vec3.add center
            (Vec3.add
            (Vec3.scale (#defocus_disk_u cam_params) (#x p))
        ...
        let
          ...
          val pixel_sample =
                Vec3.add (#pixel00_loc cam_params)
                  (Vec3.add
                    (Vec3.scale (#pixel_delta_u cam_params) u)
                    (Vec3.scale (#pixel_delta_v cam_params) v))

          val ray_orig = if #defocus_angle settings > 0.0
                        then defocus_disk_sample (#camera_center cam_params) ()
                        else #camera_center cam_params
      ...
      fun ray_color _ _ 0 = Color.create(0.0,0.0,0.0)
        ...
        let
          val recode = Hittables.hit_shape world ray (Interval.create 0.001 Real.maxFinite)
            ...
            case recode of
              ...
              | Type.Hit hit =>
                  let
                    ...
                    val emit_color =
                        case mat of
                            Type.DiffuseLightT m => DiffuseLight.emit m ray hit_record
                          | _ => Color.black

                    val scatter_res =
                        case mat of
                            Type.LambertianT m => Lambertian.scatter m ray hit_record
                          | Type.MetalT m => Metal.scatter m ray hit_record
                          | Type.DielectricT m => Dielectric.scatter m ray hit_record
                          | Type.DiffuseLightT m => DiffuseLight.scatter m ray hit_record
                  in
                    case scatter_res of
                        NONE => emit_color
                      | SOME (ray_r,col) =>
      ...
      val _ = TextIO.output (out, Int.toString (#image_width settings) ^ " " ^ Int.toString (#image_height cam_params) ^ "\n255\n")

      val _ =
        List.app (fn j =>
          let
            val _ = print("\rScanlines remaining:" ^ Int.toString ((#image_height cam_params) - j) ^ "   ")
          in
            List.app (fn i =>
              let
                val col = foldr (fn (_, acc) =>
                                  Vec3.add (ray_color (get_ray (i, j)) world (#max_depth settings)) acc)
                                Vec3.zero
                                (List.tabulate ((#samples_per_pixel settings), fn _ => ()))
                val _ = Color.write_color out (Vec3.divide col (Real.fromInt (#samples_per_pixel settings)))
              in
                ()
              end
            ) (List.tabulate ((#image_width settings), fn x => x))
          end
        ) (List.tabulate ((#image_height cam_params), fn y => y))
